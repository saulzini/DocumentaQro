...
<table  style="width: 100%" >
    ...
    <tr>
        <td>
            <img width="300" height="400" src="{{asset ($festivalItem->poster)}}">
        </td>
    <td style="width: 10%"></td>
    <td  style="width: 70%">
                <dl class="dl-horizontal">
                    @if (isset($festivalItem))
                    <dt>Festival</dt><dd>{{$festivalItem->titulo}}</dd>
                    <br>
                    ...
                    <dt>Funciones</dt>
                        @if (isset($funciones))
                             @foreach ($funciones as $funcion)
                                <dd><strong>Titulo:</strong>{{$funcion->titulo}}&nbsp;<strong>Fecha:</strong>{{$funcion->fecha}}</dd>
                            @endforeach
                        @else
                            <dd>No tiene funciones</dd>
                        @endif
                     <br>
                        ...
                        @if(isset($programas))
                            @foreach ($programas as $programa)
                                <dd>{{$programa->titulo}}</dd>
                            @endforeach
                        @else
                            <dd>No tiene programas</dd>
                        @endif
                        <br>
